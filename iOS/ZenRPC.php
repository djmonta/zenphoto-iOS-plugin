...
class ZEN_Error
{
    ...
    public function ZEN_Error($code, $message)
    {
        $this->code = $code;
...
function addZenPubData($id, $data)
{
    ...
    if ($readitem) {
        ...
        if ($readitem = null) {
            query('UPDATE '.prefix('plugin_storage').' SET `data` = '.db_quote(json_encode([
                 $parts[0] => $parts[1],
            ])).", `type` = 'iOS' WHERE `aux` = ".db_quote($id)." AND `type` = 'iOS'");
        } //$readitem = null
        else {
            $marr = array_merge($arr, [
                 $parts[0] => $parts[1],
            ]);
        ...
        query('INSERT INTO '.prefix('plugin_storage')." (`type`,`aux`,`data`) VALUES ('iOS',".db_quote($id).",'".json_encode([
             $parts[0] => $parts[1],
        ])."')");
    }
}
...
function addImageComments($args)
{
    ...
    $imageobject = getItemByID('images', $args['id']);
    $userobj = $_zp_authority->getAnAdmin([
         'user=' => $args['loginUsername'],
        'valid=' => 1,
    ]);
