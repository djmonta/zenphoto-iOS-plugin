...
class userRelated
{
    public function __construct()
    {
        ...
        if (extensionEnabled($me)) { //need to update the database tables.
            foreach ($newfields as $newfield) {
                ...
                if (query($sql, false) && in_array($newfield['table'], ['albums', 'images', 'news', 'news_categories', 'pages'])) {
    ...
    public function getOptionsSupported()
    {
        global $_zp_gallery;
        $options = [
                        gettext('Enable Facebook Integrations') => ['key'    => 'fb_integration', 'type' => OPTION_TYPE_CHECKBOX,
                                        'desc'                               => gettext('Enable or Disable Facebook Integrations'),
                                        'order'                              => 1, ],
                        gettext('Facebook App ID')					 => ['key'		          => 'fb_app_id', 'type'	 => OPTION_TYPE_TEXTBOX,
                                        'desc'	                              => gettext('Facebook App ID'),
                                        'order'	                             => 2, ],
                        gettext('Facebook App Secret')					 => ['key'		      => 'fb_app_secret', 'type'	 => OPTION_TYPE_TEXTBOX,
    ...
    {
        return [
                        ['table' => 'administrators', 'name' => 'profile_picture_url', 'desc' => gettext('Profile Picture URL'), 'type' => 'tinytext'],
                        ['table' => 'administrators', 'name' => 'fb_id', 'desc' => gettext('Facebook User ID'), 'type' => 'tinytext'],
        ];
    }

    public static function adminSave($updated, $userobj, $i, $alter)
    {
        $fields = self::fields();
        if ($userobj->getValid()) {
    ...
    public static function adminTabs($tabs)
    {
    ...
    public static function adminNotice($tab, $subtab)
    {
    ...
    public static function getCustomData($obj)
    {
        $result = [];
        $fields = self::fields();
    ...
    public static function setCustomData($obj, $values)
    {
    ...
    public static function facebook($accessToken)
    {
        if (getOption('fb_integration')) {
            $fb = new Facebook\Facebook([
              'app_id'                => getOption('fb_app_id'),
